...
async def register(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if conn:
        ...
        return templates.TemplateResponse("index.html", {"request": request, "error": "Database connection error"})
...
async def login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if conn:
        ...
        if user:
            # Generate new questions for the user
...
async def get_leaderboard_data(request: Request):
    ...
    if conn:
        cur = conn.cursor()
        cur.execute("""
            SELECT username, MAX(score) as best_score, MIN(duration) as best_duration 
            FROM leaderboard 
            GROUP BY username 
